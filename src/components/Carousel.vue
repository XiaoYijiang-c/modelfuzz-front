...
<template>
  <div
    ...
    <div :class="{'transition': transition}" :style="`width: 100%; height: 100%; will-change: transform; transform: translateX(0px);`">
      <div class="m-image" v-for="(image, index) in images" :key="index">
        <a :href="image.link ? image.link:'javascript:;'" :target="image.link ? '_blank':'_self'" class="m-link">
          <img @load="onComplete(index)" :src="image.src" :key="image.src" :alt="image.title" class="u-img" :style="`width: 100%; height: 100%;`"/>
      ...
      <div class="m-image" v-if="len">
        <a :href="images[0].link ? images[0].link:'javascript:;'" :target="images[0].link ? '_blank':'_self'" class="m-link">
          <img @load="onComplete(0)" :src="images[0].src" :key="images[0].src" :alt="images[0].title" class="u-img"  :style="`width: 100%; height: 100%;`"/>
